...
- **기간**: 2026-07-19 ~ 2026-07-21
- **프로젝트**: Tollm — 팀 단위 LLM API 공유를 위한 게이트웨이 서버
- **이번 주 목표**: 2주차 필수 기능(레이트리밋 → 쿼터 차단 → 응답 캐시 → 사용량 집계 API) 전체 구현 + 자동화 테스트 인프라 최초 구축 + 보안 자체 검수·보강
- **개발 프로세스**: 구현 → 보안 검수(SEC-01~03 발견) → 수정 → 회귀 테스트의 사이클로 진행. AI 에이전트를 역할 분담(구현/보안 검수/QA)시켜 활용하되, 모든 설계 결정의 근거는 본 보고서 3절에 직접 정리
...
| ⑧ 보안 검수 반영 3건 (SEC-01 시크릿 fail-fast / SEC-02 본문 1MB 제한 / SEC-03 쿼터 우회 차단) | ✅ 완료 + 검증 |
| src/test 자동화 테스트 인프라 최초 구축 | ✅ 완료 |

**테스트: 44/44 통과** (`./gradlew clean build`, 2026-07-21 재검증 완료, 4절)

## 2. 구현한 요청 처리 흐름 (2주차 반영)

```
POST /v1/chat/completions
  → RequestSizeLimitFilter    : [SEC-02] Content-Length 1MB 초과 시 본문 읽기 전에 413 차단
  ...
  → ProxyService.relay()
      ...
      3. 단가표(LlmModel) 등록 확인             [SEC-03] 미등록 모델은 외부 호출 전에 400 거부
      4. responseCacheService.get(cacheKey)     히트 시 즉시 반환 (cost=0, cacheHit=true 로깅)
      5. ProviderRouter → OpenAiClient/AnthropicClient (외부 호출, 1주차 기능)
      6. 응답 usage 파싱 → LlmModel 단가표로 비용 계산 → RequestLog 저장 → UsageQuota.addUsage
      7. responseCacheService.put(cacheKey, response)
...
| 단가 정보가 없는 모델은 **외부 호출 전에 400으로 거부** (SEC-03, 초기 설계에서 변경) | 최초 구현은 "라우팅 통과 후 단가가 없으면 cost=0으로 기록"이었다. 그러나 보안 검수에서 이것이 **쿼터 우회 경로**임이 지적됐다: 실제 비용은 발생하는데 UsageQuota에는 0이 누적되므로, 단가표에 없는 하위 모델명을 골라 보내면 월 한도를 사실상 무제한 우회할 수 있다. "게이트웨이가 과금을 보장하지 못하는 요청은 통과시키지 않는다"는 원칙으로 바꾸고, 비용 발생 시점(외부 호출)보다 **먼저** 단가 등록 여부를 확인해 차단하도록 수정했다 |
...
### 3.6 보안 자체 검수에서 발견·수정한 3건

구현 완료 후 보안 관점 검수를 별도로 돌려 3건을 발견하고 즉시 수정했다. "동작하는 코드"와 "안전한 코드"의 차이를 배운 과정으로, 발견 → 공격 시나리오 → 수정을 기록한다.

| ID | 발견된 문제 | 공격/사고 시나리오 | 수정 |
|---|---|---|---|
| SEC-01 | `jwt.secret`에 yml 기본값(`change-me...`)이 있어 환경변수 없이도 서버가 뜸 | 배포 환경에서 환경변수 설정을 잊으면 **공개된 기본 시크릿으로 서명된 JWT**를 누구나 위조 가능 | 기본값 제거 → 시크릿 미설정 시 부팅 자체가 실패(fail-fast). "잘못된 설정으로 조용히 뜨는 것보다 시끄럽게 죽는 게 안전" |
| SEC-02 | 프록시 요청 본문 크기 무제한 | 수십 MB 본문을 반복 전송하면 메모리 압박 + 외부 API 과금 폭탄 | `RequestSizeLimitFilter` 신설 — Content-Length 기준 1MB 초과 시 본문을 읽기 전에 413 반환 |
| SEC-03 | 단가표 미등록 모델이 비용 0으로 통과 | 단가표에 없는 모델명으로 요청하면 실비용은 발생하는데 쿼터엔 0 누적 → **월 한도 무제한 우회** | 외부 호출 전에 단가 등록 여부 확인, 미등록이면 400 거부 (3.4절) |

세 건 모두 회귀 테스트를 추가했다(`AdminAuthorizationTest` 3케이스 — /admin 인가 우회 여부 포함, `RequestSizeLimitFilterTest` 4케이스).
...
### 4.2 테스트 결과 (44/44 통과, `./gradlew clean build`)
...
| /admin 인가 (보안) | `AdminAuthorizationTest` | 3 | **MEMBER의 /admin 접근 403**, ADMIN 허용, 미인증 401 |
| 요청 크기 제한 (보안) | `RequestSizeLimitFilterTest` | 4 | 1MB 초과 413, 이하 통과, /v1 외 경로 미적용 등 |
| **합계** | | **44** | **44 통과 / 0 실패** |
...
**Q. 시크릿이 설정 안 됐을 때 "기본값으로라도 뜨는 것"과 "아예 안 뜨는 것" 중 왜 후자로 바꿨나? (SEC-01)**
기본값으로 뜨면 개발은 편하지만, 배포 환경에서 환경변수를 잊었을 때 **공개된 기본 시크릿으로 서명한 JWT를 누구나 위조할 수 있는 서버**가 조용히 운영된다. 장애는 즉시 보이지만 보안 사고는 한참 뒤에 발견된다. 그래서 잘못된 설정이면 부팅 시점에 시끄럽게 실패(fail-fast)해서 사람이 바로 고치게 만드는 쪽이 안전하다.
...
1. ~~보안/QA 검수~~ → **완료** (3.6절의 SEC-01~03 발견·수정, 회귀 테스트 7건 추가까지 반영)
